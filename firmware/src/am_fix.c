...
// -89dBm target (battle-tested value used by OneOfEleven/egzumer/F4HWN).
// Lower targets reduce gain more aggressively, which can drop RSSI below the
// squelch threshold and prevent audio from opening on AM.
...
void AM_fix_10ms(const unsigned vfo)
{
    ...
    if (diff_dB > 0) {  // decrease gain
        ...
        if (gain_table_index[vfo] != index)
        {
            ...
            hold_counter[vfo] = 5;        // 50ms hold (fast attack for airband)
        }
    }

    if (diff_dB >= -6)                    // 6dB hysterisis (help reduce gain hunting)
        hold_counter[vfo] = 50;           // 500ms decay hold (slow release reduces noise between transmissions)
