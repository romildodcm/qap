...
//          gRequestSaveChannel = 1;
        ...
        case MENU_STARTUP:
            AIRBAND_SetStartupMode(gSubMenuSelection ? STARTUP_SCAN : STARTUP_LAST);
            return;

        case MENU_OPMODE:
            AIRBAND_SetOpMode((op_mode_t)gSubMenuSelection);
            return;
...
void MENU_ShowCurrentSetting(void)
{
    switch (UI_MENU_GetCurrentMenuId())
    {
        ...
        case MENU_STARTUP:
            gSubMenuSelection = (gAirbandConfig.startup_mode == STARTUP_SCAN) ? 1 : 0;
            break;

        case MENU_OPMODE:
            gSubMenuSelection = gAirbandConfig.last_op_mode;
            break;
