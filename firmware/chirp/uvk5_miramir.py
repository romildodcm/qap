...
DRIVER_VERSION = "Quansheng UV-K5/K6/5R driver ver: 2024/10/03 (c) EGZUMER + F4HWN v3.5.0"
...
PDF_FILE_ALAIN_UPDATE = "https://www.dropbox.com/scl/fi/hcitzq5anpbho7vreyil4/MENU-FIRMWARE-F4HWN-v3.4.pdf?rlkey=k1x4mnlrvyod2brxolhxh070f&e=1&dl=0"
...
# bugfix calibration : put in comment next line: upload_calibration = False
    ...
    def get_memory(self, number):
        ...
        rs.set_doc('Scramb: Do you want the scrambler on this frequency?')
        mem.extra.append(rs)
...
# end add menu firmware with version and option display
        ...
        rs = RadioSetting("sl3PriorEnab", "List 3 Priority Channel Scan", val)
        rs.set_doc('List 3 priority: Is this list as priority')
        scanl.append(rs)

        tmpch = list_def(_mem.sl3PriorCh1 + 1, ch_list, 0)
        val = RadioSettingValueList(ch_list, None, tmpch)
        rs = RadioSetting("sl3PriorCh1", "List 3 Priority Channel 1", val)
        rs.set_doc('List 3 priority channel 1: Select the channel you want for priority')
        scanl.append(rs)

        tmpch = list_def(_mem.sl3PriorCh2 + 1, ch_list, 0)
        val = RadioSettingValueList(ch_list, None, tmpch)
        rs = RadioSetting("sl3PriorCh2", "List 3 Priority Channel 2", val)
        rs.set_doc('List 3 priority channel 2: Select the channel you want for priority')
        scanl.append(rs)
...
#       message box to the web page link (chirp)
        ...
        append_label(roinfo, "The driver module is done by VE2ZJM","The first french translation was done by FLIXX")
        append_label(roinfo,"","" )
        
        append_label(roinfo,
                     "=" * 6 + " PDF File for Firmware F4HWN " + "=" * 300, "=" * 300)

        append_label(roinfo, "PDF File","The PDF file is done by 14UVR010")
...
#       message box to the web page link (PDF)
        ...
        def validate_Go_Web_Page2(value):
            if value :
                msg = "Go to the web Page of the full explanation for Firmware F4HWN \n" \
        ...
        rs = RadioSetting("Update_Pdf_0","Go to the web Page of the full explanation for Firmware F4HWN select this Box ->", val)
        rs.set_doc('Be sure you have the latest PDF File to get all info!')
        roinfo.append(rs)

        append_label(roinfo,"" ,"" )

#       message box to the web page link (PDF)
        append_label(roinfo, "QUICK KEY File","The Quick key file is done by IK5WWG" )
        val = RadioSettingValueBoolean(False)
        def validate_Go_Web_Page3(value):
            if value :
                msg = "Go to the web Page of the QUICK KEY For Firmware F4HWN \n" \
                      + PDF_FILE_QUICK_KEY
                ret = wx.MessageBox(msg, "Warning", wx.OK | wx.CANCEL |
                                    wx.CANCEL_DEFAULT | wx.ICON_WARNING)
                if ret == wx.OK :
        ...
        rs = RadioSetting("Update_Pdf_3","Go to the web Page of the QUICK KEY For Firmware F4HWN select this Box ->", val)
        rs.set_doc('Go see the quick Key info!')
        roinfo.append(rs)
...
#       message box to the web page link (youtube)
        append_label(roinfo,
                     "=" * 6 + " YOUTUBE CHANNEL of F4HWN " + "=" * 300, "=" * 300)

        val = RadioSettingValueBoolean(False)
        def validate_Go_Web_Page3(value):
            if value :
                msg = "Go to the youtube channel of F4HWN \n" \
                      + CHAINE_F4HWN
                ret = wx.MessageBox(msg, "Warning", wx.OK | wx.CANCEL |
                                    wx.CANCEL_DEFAULT | wx.ICON_WARNING)
                if ret == wx.OK :
        ...
        val.set_validate_callback(validate_Go_Web_Page3)
        rs = RadioSetting("CHAINE_YOUTUBE","Go to the youtube channel of F4HWN select this Box ->", val)
        rs.set_doc('For video explain firmware F4HWN')
        roinfo.append(rs)
...
#       message box to the web page link (youtube allemand)
        append_label(roinfo,
                     "=" * 6 + " SOME YOUTUBE LINK INFO IN SOME LANGUAGE FOR F4HWN " + "=" * 300, "=" * 300)
...
#       1 youtube link #YOUTUBE_INFO_FUNKWELLE
        ...
        def validate_Go_Web_Page3(value):
            if value :
                msg = "Open Youtube web page from FUNKWELLE in GERMAN \n" \
        ...
        val.set_validate_callback(validate_Go_Web_Page3)
        rs = RadioSetting("CHAINE_YOUTUBE3","Youtube from FUNKWELLE (German)   select this Box ->", val)
        rs.set_doc('For video explain firmware F4HWN')
        roinfo.append(rs)
...
#       2 youtube channel YOUTUBE_INFO_M7FRS
        ...
        def validate_Go_Web_Page4(value):
            if value :
                msg = "Open Youtube web page from M7FRS \n" \
                      + YOUTUBE_INFO_M7FRS
                ret = wx.MessageBox(msg, "Warning", wx.OK | wx.CANCEL |
                                    wx.CANCEL_DEFAULT | wx.ICON_WARNING)
                if ret == wx.OK :
        ...
        rs = RadioSetting("CHAINE_YOUTUBE4","Youtube from M7FRS   select this Box ->", val)
        rs.set_doc('For video explain firmware F4HWN')
        roinfo.append(rs)
...
#       3 youtube channel YOUTUBE_INFO_PU4BLA
        ...
        def validate_Go_Web_Page5(value):
            if value :
                msg = "Open Youtube web page from PU4BLA \n" \
                      + YOUTUBE_INFO_PU4BLA
                ret = wx.MessageBox(msg, "Warning", wx.OK | wx.CANCEL |
                                    wx.CANCEL_DEFAULT | wx.ICON_WARNING)
                if ret == wx.OK :
        ...
        rs = RadioSetting("CHAINE_YOUTUBE5","Youtube from PU4BLA   select this Box ->", val)
        rs.set_doc('For video explain firmware F4HWN')
        roinfo.append(rs)
...
#       4 youtube channel YOUTUBE_INFO_R3MDG
        ...
        def validate_Go_Web_Page6(value):
            if value :
                msg = "Open Youtube web page from R3MDG \n" \
                      + YOUTUBE_INFO_R3MDG
                ret = wx.MessageBox(msg, "Warning", wx.OK | wx.CANCEL |
                                    wx.CANCEL_DEFAULT | wx.ICON_WARNING)
                if ret == wx.OK :
        ...
        rs = RadioSetting("CHAINE_YOUTUBE6","Youtube from R3MDG   select this Box ->", val)
        rs.set_doc('For video explain firmware F4HWN')
        roinfo.append(rs)

#       5 youtube channel YOUTUBE_INFO_PY2MAJ_BRAZIL
        val = RadioSettingValueBoolean(False)
        def validate_Go_Web_Page7(value):
            if value :
                msg = "Open Youtube web page from PY2MAJ_BRAZIL \n" \
                      + YOUTUBE_INFO_PY2MAJ_BRAZIL
                ret = wx.MessageBox(msg, "Warning", wx.OK | wx.CANCEL |
                                    wx.CANCEL_DEFAULT | wx.ICON_WARNING)
                if ret == wx.OK :
                    webbrowser.open(YOUTUBE_INFO_PY2MAJ_BRAZIL)
                value = False
                    
            return value

        val.set_validate_callback(validate_Go_Web_Page7)
        rs = RadioSetting("CHAINE_YOUTUBE7","Youtube from PY2MAJ_BRAZIL   select this Box ->", val)
        rs.set_doc('For video explain firmware F4HWN')
        roinfo.append(rs)

#       6 youtube channel YOUTUBE_INFO_HA8CSY_HUNGARIA
        val = RadioSettingValueBoolean(False)
        def validate_Go_Web_Page8(value):
            if value :
                msg = "Open Youtube web page from HA8CSY_HUNGARIA \n" \
                      + YOUTUBE_INFO_HA8CSY_HUNGARIA
                ret = wx.MessageBox(msg, "Warning", wx.OK | wx.CANCEL |
                                    wx.CANCEL_DEFAULT | wx.ICON_WARNING)
                if ret == wx.OK :
                    webbrowser.open(YOUTUBE_INFO_HA8CSY_HUNGARIA)
                value = False
                    
            return value

        val.set_validate_callback(validate_Go_Web_Page8)
        rs = RadioSetting("CHAINE_YOUTUBE8","Youtube from HA8CSY_HUNGARIA   select this Box ->", val)
        rs.set_doc('For video explain firmware F4HWN')
        roinfo.append(rs)
